...
if __name__ == '__main__':
    ...
    latch_pop = sim.Population(1, sim.IF_curr_exp(**neuron_params), initial_values={'v': neuron_params["v_rest"]}, label="latch")
    ...
    sim.Projection(and_pop, latch_pop, sim.OneToOneConnector(), std_conn)
    sim.Projection(delay_pop, latch_pop, sim.OneToOneConnector(), std_conn, receptor_type="inhibitory")
    sim.Projection(latch_pop, and_pop, sim.OneToOneConnector(), std_conn, receptor_type="inhibitory")

    # Recurrence
    sim.Projection(sim.PopulationView(latch_pop, [0]), sim.PopulationView(latch_pop, [0]), sim.OneToOneConnector(), std_conn)
