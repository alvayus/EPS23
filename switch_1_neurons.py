...
if __name__ == '__main__':
    ...
    switch_pop = sim.Population(2, sim.IF_curr_exp(**neuron_params), initial_values={'v': neuron_params["v_rest"]}, label="switch")

    # - Connections -
    # Inputs
    sim.Projection(src_switch, sim.PopulationView(switch_pop, [0]), sim.OneToOneConnector(), std_conn)
    sim.Projection(src_switch, sim.PopulationView(switch_pop, [1]), sim.OneToOneConnector(), std_conn, receptor_type="inhibitory")

    # Interconnections
    sim.Projection(sim.PopulationView(switch_pop, [0]), sim.PopulationView(switch_pop, [1]), sim.OneToOneConnector(), std_conn)
    sim.Projection(sim.PopulationView(switch_pop, [1]), sim.PopulationView(switch_pop, [0]), sim.OneToOneConnector(), std_conn, receptor_type="inhibitory")
    ...
    sim.Projection(sim.PopulationView(switch_pop, [1]), sim.PopulationView(switch_pop, [1]), sim.OneToOneConnector(), std_conn)
